...
commands.log:
	rm -f commands.log
	for f in src/*.h src/internal/*.h src/internal/networking/*.h; do \
		echo $(CC) $(CFLAGS) -flto -c -o "$$f.gch" "$$f" >> commands.log; \
	done
	for f in src/*.c src/eventing/*.c src/crypto/*.c; do \
		echo $(CC) $(CFLAGS) -flto -c -o $$(basename "$$f" ".c").o "$$f" >> commands.log; \
	done
	for f in src/internal/eventing/*.h; do \
		echo $(CXX) $(CXXFLAGS) -std=c++17 -flto -Isrc -c -o "$$f.gch" "$$f" >> commands.log; \
	done
	for f in src/crypto/*.cpp; do \
		echo $(CXX) $(CXXFLAGS) -std=c++17 -flto -c -o $$(basename "$$f" ".cpp").o "$$f" >> commands.log; \
	done
	for f in examples/*.c; do \
		echo $(CC) -flto $(CFLAGS) -c -o $$(basename "$$f" ".c").o "$$f" >> commands.log; \
	done
	echo $(CXX) $(CXXFLAGS) -std=c++14 -flto -Isrc -O3 -c -o asio.o src/eventing/asio.cpp >> commands.log;
